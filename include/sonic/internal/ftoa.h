...
typedef struct uint64x2 {
  uint64_t hi;
  uint64_t lo;
} uint64x2;

typedef struct F64Decimal {
  uint64_t sig;
  int64_t exp;
} F64Decimal;

static sonic_force_inline const uint64x2 Pow10CeilSig(int32_t k) {
...
static sonic_force_inline uint64_t RoundToOdd(uint64x2 g, uint64_t cp) {
...
static sonic_force_inline uint32_t Lo32(uint64_t x) {
  return static_cast<uint32_t>(x);
}

static sonic_force_inline uint32_t Hi32(uint64_t x) {
  return static_cast<uint32_t>(x >> 32);
}

static sonic_force_inline uint64x2 Mul128(uint64_t a, uint64_t b) {
...
static sonic_force_inline unsigned Ctz10(const uint64_t v) {
  if (v >= 10000000000ull) {
...
static sonic_force_inline char* FormatSignificand(uint64_t sig, char* out,
  ...
  if ((sig >> 32) != 0) {
    ...
    if (r != 0) {
      ...
      uint32_t d0 = (d % 100) << 1;
      uint32_t d1 = (d / 100) << 1;
      Copy2Digs(p - 2, kDigits + c0);
      Copy2Digs(p - 4, kDigits + c1);
      Copy2Digs(p - 6, kDigits + d0);
      Copy2Digs(p - 8, kDigits + d1);
    } else {
      ctz += 8;
    }
    p -= 8;
...
static sonic_force_inline bool IsDivPow2(uint64_t val, int32_t e) {
...
static sonic_force_inline char* FormatExponent(F64Decimal v, char* out,
  ...
  *end++ = 'e';
  ...
  if (exp >= 100) {
    int32_t c = exp % 10;
    Copy2Digs(end, kDigits + 2 * (exp / 10));
    end[2] = (char)('0' + c);
    end += 3;
  } else if (exp >= 10) {
    Copy2Digs(end, kDigits + 2 * exp);
    end += 2;
  } else {
    *end++ = (char)('0' + exp);
  }
...
static sonic_force_inline char* FormatDecimal(F64Decimal v, char* out,
                                              unsigned cnt) {
  char* p = out;
  char* end;
  int point = cnt + v.exp;
  ...
  /* add the remaining kDigits */
  end = FormatSignificand(v.sig, p, cnt);
  while (*(end - 1) == '0') end--;
  if (point <= 0) {
    return end;
  }
  ...
  int digs = end - p;
  if (digs > point) {
    std::memmove(p + point + 1, p + point, digs - point);
    p[point] = '.';
    end++;
  } else {
    size_t nzeros = point - digs;
    std::memset(end, '0', nzeros + 2);
    end[nzeros] = '.';
    end += nzeros + 2;
  }
  return end;
...
static sonic_force_inline uint64_t F64ToRaw(double fp) {
  union {
    uint64_t u64;
    double f64;
  } uval;
...
/**
 ...
 The string format of float point is as follows:
 https://tc39.es/ecma262/#sec-numeric-types-number-tostring
 */
static sonic_force_inline F64Decimal F64ToDecimal(uint64_t rsig, int32_t rexp,
                                                  uint64_t c, int32_t q) {
  ...
  /*  k is in [-1233, 1233]
      ...
      vb = 4V = 4 * c * 2^q * 10^-k */
  h = q + ((-k) * 1741647 >> 19) + 1;
  uint64x2 pow10 = Pow10CeilSig(-k);
  vbl = RoundToOdd(pow10, cbl << h);
  vb = RoundToOdd(pow10, cb << h);
  vbr = RoundToOdd(pow10, cbr << h);
...
sonic_static_noinline int F64toa(char* out, double fp) {
  ...
  if (sonic_unlikely(rexp == F64_INF_NAN_EXP)) {
    return 0;
  }
